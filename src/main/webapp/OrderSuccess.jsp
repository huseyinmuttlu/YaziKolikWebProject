...
<div class="container">
    ...
    <div class="accordion accordion-flush" id="accordionFlushExample">
        <!--Accordion Item Her sipariş için bu tekrar edecek. 1.for-->
        <div class="accordion-item">
          <h2 class="accordion-header" id="flush-headingOne">
              <!--Alttaki button objesinin "data-bs-target" ve "aria-controls" değeri ile alttaki id değeri "flush-collapseone" olan div aynı olacak-->
            <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapseOne" aria-expanded="false" aria-controls="flush-collapseOne">
              <div class="row row-cols-3 w-100">
                  <div class="col">
                      <p>Sipariş No: ${order.getOrderID()}</p>
                  </div>
                  <div class="col"><p>Sipariş Tutarı: ${order.getPaymentAmount()} </p></div>
                  <div class="col">
                      <p>Sipariş Tarihi:${order.getOrderDate()}</p>
                  </div>
              </div>
            </button>
          </h2>
          <!--id değeri flush-collapseone olan div , "aria-labelledby" değeri ile class = "accordion-header" kısmının id değeri aynı olacak-->
          <div id="flush-collapseOne" class="accordion-collapse collapse" aria-labelledby="flush-headingOne" data-bs-parent="#accordionFlushExample">
            <div class="accordion-body"><!--İkinci for'dan gelen veriler buraya eklenecek 2.for buradan başlayacak-->
                   <c:forEach items="${orderDetails}" var="orderDetail">
                <div class="row row-cols-4 my-2 border py-2">
                    <div class="col">
                        <img src="${orderDetail.getProduct().getImageUrl() }" alt=""> <!--Kitap Resmi-->
                    </div>
                    <div class="col">
                        <p>${orderDetail.getProduct().getTitle()}</p>
                    </div>
                    <div class="col">
                        <p>${orderDetail.getQuantity()}</p>
                    </div>
                    <div class="col">
                        <p>${orderDetail.getTotalAmaount()}</p>
                    </div>
                </div>
                <!--ikinci for bitişi-->
                </c:forEach>
